...
def render_sets(model, net, opt, epoch:int, args):
    with torch.no_grad():
        model.test_folder = model.test_folder.replace('assets/test_pose', f'{args.source_path}/train')
        avatarmodel = AvatarModel(model, net, opt, train=False)
        avatarmodel.training_setup()
        avatarmodel.load(epoch)
        
        recon_pose_dataset = avatarmodel.getReconDataset()
        recon_pose_loader = torch.utils.data.DataLoader(recon_pose_dataset,
                                            batch_size = 1,
                                            shuffle = False,
                                            num_workers = 4,)

        render_path = os.path.join(avatarmodel.model_path, 'pose_reconstruct', "ours_{}".format(epoch))
        makedirs(render_path, exist_ok=True)
        for idx, batch_data in enumerate(tqdm(recon_pose_loader, desc="Rendering progress")):
